...
namespace Application.Features.Auth.Register
{
    public class RegisterCommandValidator : AbstractValidator<RegisterCommand>
    {
        public RegisterCommandValidator()
        {
            ...
            //.WithMessage(ValidationMessages.Required).WithName("Soyad")
            ...
            //.WithMessage(ValidationMessages.Required).WithName("Telefon numarası")
            //.Matches(@"^\+?\d{10,15}$").WithMessage(ValidationMessages.InvalidFormat).WithName("Telefon numarası");

            RuleFor(r => r.Password).NotEmpty();
                //.WithMessage(ValidationMessages.Required).WithName("Şifre")
